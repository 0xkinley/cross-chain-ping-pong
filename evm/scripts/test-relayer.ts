...
/**
 * EVM Cross-Chain Relayer Test Script
 * 
 * This script tests the EVM relayer functionality without running the full loop.
 * It verifies that we can connect to the deployed EVM contract and read the game state.
 */
...
async function testEVMRelayer() {
  ...
  try {
    ...
    if (balance < ethers.parseEther("0.001")) {
      console.warn("⚠️  Low balance - may need more ETH for LayerZero fees");
    }
    ...
    if (!isInitialized) {
      ...
      console.log("Call serve() to start the ping-pong game");
        ...
        try {
          ...
          const quote = await contract.quote(1000n, 1n, options, payInLzToken); // Test with sample values
          ...
          console.warn("⚠️  Insufficient balance for cross-chain message");
        }
      } catch (error: any) {
        console.warn("⚠️  Fee quote failed:", error.message);
      }
    } else {
      console.log("\n🎾 Game is in progress!");
